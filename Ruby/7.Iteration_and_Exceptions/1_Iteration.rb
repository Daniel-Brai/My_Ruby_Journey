# 1: Each
# Ruby has many types of enumerators but the first and most simple type of enumerator to start with is each. We will
...
# 2. .each is an enumerator that enumerates over each element in the object it is acting on. In this case, it acts on
#     each number in the range.

# 3. { |i| puts i.even? ? 'even' : 'odd' } is the block for the each statement, which itself can be broken
#     down further.
#     1. |i| this means that each element in the range is represented within the block by the identifier i.
    
#     2. puts is an output method in Ruby that has an automatic line break after each time it prints. (We can
#     use print if we don't want the automatic line break)
    
#     3. i.even? checks if i is even. We could have also used i % 2 == 0; however, it is preferable to use built
#     in methods.
    
#     4. ? "even" : "odd" this is ruby's ternary operator. The way a ternary operator is constructed is
...
# 2: Implementation in a class
# Enumerable is the most popular module in Ruby. 
# Its purpose is to provide you with iterable methods like map, SELECT, reduce, etc. 
# Classes that use Enumerable include Array, Hash, Range. To use it, you have to include Enumerable and implement each.
...
# 3: Iterating over complex objects
# Arrays
...
# 5: Iteration with index
# Sometimes you want to know the position (index) of the current element while iterating over an enumerator. 
# For such purpose, Ruby provides the with_index method. 
# It can be applied to all the enumerators. 
# Basically, by adding with_index to an enumeration, you can enumerate that enumeration. 
# Index is passed to a block as the second argument.
...
# The only difference between it and each.with_index is that you can't pass an argument to that, so the first index is 0 all the time.
