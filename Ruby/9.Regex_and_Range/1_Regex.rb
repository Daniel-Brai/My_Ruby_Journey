...
# Though 'haystack' =~ /hay/ is in most cases an equivalent, side effects might differ:
...
# Note that this does not return a true/false value, it instead returns either the index of the match if found, or nil if not found. 
# Because all integers in ruby are truthy (including 0) and nil is falsy, this works. If you want a boolean value, use
# #===
# as shown in another example.

# 2: Regular Expressions in Case Statements
...
# This works because case statements are checked for equality using the === operator, not the == operator. 
# When a regex is on the left hand side of a comparison using ===, it will test a string to see if it matches.

# 3: Groups, named and otherwise
# Ruby extends the standard group syntax (...) with a named group, (?<name>...). 
...
By default, quantifiers are greedy, which means they take as many characters as they can while still making a
...
Exchange' is an optional part of the string (because it could be 'Stack Overflow' instead), the naive solution will not
work as expected:
GoalKicker.com – Ruby® Notes for Professionals 113
/(?<site>.*)( Stack Exchange)?/
...
More advanced but still short and quick: search for a specific group by using the second parameter, 2 is the second
...
variables. If the second parameter is present, it specifies the position in the string to begin the search.
/R.../.match?("Ruby") #=> true
...
GoalKicker.com – Ruby® Notes for Professionals 114
using slashes: / /
using %r{}
using Regex.new
...
GoalKicker.com – Ruby® Notes for Professionals 115
\b # Any word boundary
