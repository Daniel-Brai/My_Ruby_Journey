...
# 1: CLASS VARIABLES
# Class variables have a class wide scope, they can be declared anywhere in the class.
...
# dino = Dinosaur.new
# puts dino.classification # => "Like a Reptile, but like a bird"
...
# puts Dinosaur.classification # => "Big teeth bird!"
...
puts DuckDinosaur.class_variables # => [:@@classification]
puts SomethingStrange.class_variables # => [:@@classification]

puts DuckDinosaur.classification # => "Big teeth bird!
...
# Its scope is dependent on where it has been declared, 
# it can not be used outside the "declaration containers" scope. 
# For example, if a local variable is declared in a method, it can only be used inside that method.
def some_method
...
# Of course, local variables are not limited to methods, 
# as a rule of thumb you could say that, as soon as you declare
# a variable inside a do ... end block or wrapped in curly braces {} it will be local and scoped to the block it has been declared in.
...
# The variables used for block arguments are (of course) local to the block, but will overshadow previously defined
...
# Their scope is not dependent on where they are defined. 
# A variable will be considered global, when prefixed with a $ sign.
...
# Since a global variable can be defined everywhere and will be visible everywhere, calling an "undefined" global
...
# Instance variables have an object wide scope, 
# they can be declared anywhere in the object, however an instance
# variable declared on class level, will only be visible in the class object. 
# A variable will be considered an instance variable when prefixed with @. 

# Instance variables are used to set and get an objects attributes and will return nil if not defined.
